...
import { CheckCircle, Clipboard, DoubleDown, NoSymbol } from './IconComponents';
...
const Mm2LogsPanel = ({ windowSizes, setWindowSizes }) => {
  ...
  const [shouldAlwaysScrollToBottom, setShouldAlwaysScrollToBottom] = useState(true);
      ...
      <div className="w-full p-2 flex-[0_0_auto] bg-primaryLight text-[#a2a3bd] h-10 border-b border-b-gray-800">
        <div className="flex justify-between items-center">
          <div className="flex gap-3 items-center">
            <Tooltip label={windowSizes.bottomBar <= 40 ? 'Expand panel' : 'Collapse Panel'}>
              <DoubleDown
                onClick={() => {
                  setWindowSizes({
                    ...windowSizes,
            ...
            {!copied && (
              <Tooltip label={'Copy Logs'}>
                <Clipboard
                  onClick={() => {
                    copyToClipboard(
                      mm2LogsPanelState.outputMessages.map((log) => log[0]).join('\n'),
                    );
                    setCopied(true);
          ...
          return (
            <p
              ...
              className={`whitespace-pre-wrap text-${message[1]}-300 ${
                isInlineCopied.id === index && 'text-green-600 hover:text-green-600'
